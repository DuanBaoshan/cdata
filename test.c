...
static DBList_t g_normalList;
static DBList_t g_studentList;
...
static Testcase_t g_testcaseArray[] =
{
	...
	{"Test multi thread with DBList_Lock", TestMultiThreadLock},
};
...
static int InitList()
{
    ...
    ret = DBList_Create("NormalDataList", sizeof(int), &g_normalList);
    if (ret != ERR_OK)
    {
    ...
    ret = DBList_CreateRef("StudentList", &g_studentList);
    if (ret != ERR_OK)
    {
        LOG_E("Fail to create normal list.\n");
        return -1;
	...
	DBList_SetFreeDataFunc(g_studentList, FreeStudentData);
...
static int DestroyList()
{
    ...
    ret = DBList_Destory(g_normalList);
    if (ret != ERR_OK)
    {
    ...
    ret = DBList_Destory(g_studentList);
    if (ret != ERR_OK)
    {
...
static int ShowIntList(DBList_t list)
{
    printf("=============%s Begin==================\n", DBList_Name(list));
    DBList_Traverse(list, NULL, TraverseIntList);
    printf("=============%s End==================\n", DBList_Name(list));
	
	return 0;
}

static int ShowIntListReversely(DBList_t list)
{
    printf("=============%s Begin==================\n", DBList_Name(list));
    DBList_TraverseReversely(list, NULL, TraverseIntList);
    printf("=============%s End==================\n", DBList_Name(list));
...
static int TestNormalList()
{
    ...
    {
        value = i + 1;
        DBList_InsertData(g_normalList, &value);
    }
    printf("\n==>After inster data:\n");	
	...
	p_data = (int*)DBList_GetDataAtPos(g_normalList, 0);
	if (p_data == NULL)
	{
    ...
    DBList_Clear(g_normalList);
    printf("After clear, list count:%d.\n", (int)DBList_Count(g_normalList));

	//Test DBList_InsertData2Head
	count = 10;    
    LOG_A("count:%d.\n", count);    
    for (i = 0; i < count; i++)
    {
        value = i + 1;
        DBList_InsertData2Head(g_normalList, &value);
    }	
	ShowIntList(g_normalList);	
	DBList_Clear(g_normalList);
	
	//Test DBList_InsertDataAscently
    ...
    {
        value = i + 1;
        DBList_InsertDataAscently(g_normalList, &value, IntLtListData);
    }	
	ShowIntList(g_normalList);
	DBList_Clear(g_normalList);
	
	//Test DBList_InsertDataDescently
    ...
    {
        value = i + 1;
        DBList_InsertDataDescently(g_normalList, &value, IntLtListData);
    }
    ShowIntList(g_normalList);
	DBList_Clear(g_normalList);	
    return 0;
...
static int TestInsertDataAtPos()
{
	...
	int i = 0;
    int value = 0;
	DBListNode_t node = NULL;	
	
	for (i = 0; i < 5; i++)
	{
		value = i + 1;
		node = DBList_InsertData(g_normalList, &value);
		if (node == NULL)
		{
	...
	node = DBList_InsertDataAtPos(g_normalList, &value, 0);
    if (node == NULL)
    {
	...
	value = 100;
	LOG_A("Wil insert %d at %d.\n", value, (int)(DBList_Count(g_normalList) - 1));
	node = DBList_InsertDataAtPos(g_normalList, &value, DBList_Count(g_normalList) - 1);
    if (node == NULL)
    {
        LOG_E("Fail to insert at position.\n");
        errRet = -1;
	...
	node = DBList_InsertDataAtPos(g_normalList, &value, 2);
    if (node == NULL)
    {
        LOG_E("Fail to insert at position.\n");
        errRet = -1;
...
static int TestStopTraverseNormalData()
{
	...
	DBList_Traverse(g_normalList, &userData, TraverseStopIntList);
	
	printf("\nWill find the data which < userData:%d reversely.\n", userData);
	DBList_TraverseReversely(g_normalList, &userData, TraverseStopIntList);
...
static int TestSwapPos()
{
	...
	DBListNode_t node1;
	value = 2;	
	ret = DBList_CreateNode(g_normalList, &value, &node1);
	if (ret != ERR_OK)
	{
	...
	LOG_A("node1:%p, next:%p, pre:%p.\n", node1, DBList_GetNextNode(node1), DBList_GetPreNode(node1));
	
	ret = DBList_InsertNode(g_normalList, node1);
	if (ret != ERR_OK)
	{
	    LOG_E("Fail to insert node.\n");
		
		DBList_DestroyNode(node1, NULL);
        errRet = -1;
		goto EXIT;
	}
	LOG_A("node1:%p, next:%p, pre:%p.\n", node1, DBList_GetNextNode(node1), DBList_GetPreNode(node1));
	...
	DBListNode_t node2;
	value = 4;	
	ret = DBList_CreateNode(g_normalList, &value, &node2);
	if (ret != ERR_OK)
	{
	    LOG_E("Fail to create node.\n");		
        errRet = -1;
	...
	LOG_A("node2:%p, next:%p, pre:%p.\n", node2, DBList_GetNextNode(node2), DBList_GetPreNode(node2));
	
	ret = DBList_InsertNode(g_normalList, node2);
	if (ret != ERR_OK)
	{
	    LOG_E("Fail to insert node.\n");
		
		DBList_DestroyNode(node2, NULL);
        errRet = -1;
		goto EXIT;
	}
	LOG_A("node2:%p, next:%p, pre:%p.\n", node2, DBList_GetNextNode(node2), DBList_GetPreNode(node2));
	...
	DBList_SwapPos(g_normalList, node2, node1);
...
static int TestSwapNeighbourHeadTail()
{
	...
	DBList_SwapPos(g_normalList, node1, node2);
...
static DBListNode_t CreateStudentNode(const char* p_name, 	
								char sex,
								int age,
								int chineseScore,								
								int mathScore,
								int englishScore)
{
	...
	DBListNode_t node = NULL;
	if (DBList_CreateNode(g_studentList, p_student, &node) != ERR_OK)
	{
		LOG_E("Fail to create node.\n");
...
static int ShowStudentList(DBList_t list)
{
	printf("====================%s Begin============================\n", DBList_Name(list));
	DBList_Traverse(list, NULL, ShowStudent);
	printf("====================%s End============================\n\n", DBList_Name(list));
	return 0;
}
...
{
	DBList_Clear(g_studentList);
	DBListNode_t node = NULL;
	
	node = CreateStudentNode("Jack", 'M', 10, 78, 82, 67);
	DBList_InsertNodeDescently(g_studentList, node, TotalScoreDescent);	
	...
	DBList_InsertDataDescently(g_studentList, p_student, TotalScoreDescent);	
...
static int TestNormalStructureList()
{
	...
	int ret = DBList_Create("StructureList", sizeof(Student_t), &list);	
	if (ret != ERR_OK)
	{
		LOG_E("Fail to create list.\n");
		return -1;
	}
	DBList_SetFreeDataFunc(list, FreeStudentData);
	...
	node = DBList_InsertData(list, &student);
	...
	LOG_I("Count:%d.\n", (int)DBList_Count(list));
	
	printf("==>Show student list from head to tail:\n");
	printf("===============%s begin=============================\n", DBList_Name(list));
	FOR_EACH_IN_DBLIST(node, list)
	{
		p_student = (Student_t*)DBList_GetNodeData(node);
...
static int TestReferenceList()
{
	...
	// Test DBList_GetMachCount
	CdataCount_t count = 0;
	DBList_GetMachCount(g_studentList, &condition, FindJohn, &count);
	LOG_A("Find boys with name '%s', count:%u.\n", condition.name, count);
	
	//Test DBList_DataExists
	memset(condition.name, 0, sizeof(condition.name));
	strcpy(condition.name, "John");
	condition.age = 11;
	if (DBList_DataExists(g_studentList, &condition, FindJohn))
	{
	...
	DBListNode_t node = DBList_GetFirstMatchNode(g_studentList, &condition, FindJohn);
	if (node != NULL)
	{
		p_student = (Student_t *)DBList_GetNodeData(node);
	...
	DBList_InsertDataBefore(g_studentList, node, p_student);
	LOG_A("After insert before:\n");
	ShowStudentList(g_studentList);
	
	p_student = CreateStudent("AfterStu", 'M', 13, 89, 70, 94);
	DBList_InsertDataAfter(g_studentList, node, p_student);
...
static int TestDetach()
{
	...
	condition.age = 11;	
	LOG_A("Condition: name:%s, sex:'%c', age:%d.\n", condition.name, condition.sex, condition.age);	
	node = DBList_GetFirstMatchNode(g_studentList, &condition, FindJohn);
	if (node != NULL)
	{
		p_student = (Student_t *)DBList_GetNodeData(node);

		DBList_DetachNode(g_studentList, node);	
		LOG_A("After detach stduent(Name:%s, sex:'%c', age:%d) from student list.\n", p_student->p_name, p_student->sex, p_student->age);
		ShowStudentList(g_studentList);	
		DBList_DestroyNode(g_studentList, node);		
	}		
	
	node = DBList_DetachNodeAtPos(g_studentList, 1);
	p_student = (Student_t*)DBList_DetachNodeData(node);
	DBList_DestroyNode(g_studentList, node);	
	...
	char sex = 'M';
	p_student = DBList_DetachData(g_studentList, &sex, FindBySex);
	LOG_A("After detach first 'M', name:%s, age:%d.\n", p_student->p_name, p_student->age);	
	ShowStudentList(g_studentList);	
	FreeStudentData(p_student);
	
	node = DBList_DetachHead(g_studentList);
	p_student = DBList_GetNodeData(node);
	LOG_A("Detach head:%s, '%c', %d.\n", p_student->p_name, p_student->sex, p_student->age);
	DBList_DestroyNode(g_studentList, node);	
	LOG_A("After detach head.\n");	
	ShowStudentList(g_studentList);	

	node = DBList_DetachTail(g_studentList);
	p_student = DBList_GetNodeData(node);
...
static int TestRm()
{
	...
	if (node != NULL)
	{
		...
		DBList_RmNode(g_studentList, node);			
		ShowStudentList(g_studentList);			
	}	
	
	
	DBList_RmHead(g_studentList);
	LOG_A("After rm head:\n");
	ShowStudentList(g_studentList);	

	DBList_RmTail(g_studentList);
	LOG_A("After rm tail:\n");
	ShowStudentList(g_studentList);	
	
	DBList_RmNodeAtPos(g_studentList, 1);
	LOG_A("After rm from position 1:\n");
	ShowStudentList(g_studentList);	
	
	char sex = 'M';
	DBList_RmFirstMatchNode(g_studentList, &sex, FindBySex);
	LOG_A("After rm firt match sex 'M':\n");
	ShowStudentList(g_studentList);		
	
	sex = 'F';
	DBList_RmAllMatchNodes(g_studentList, &sex, FindBySex);
...
static int TestMatch()
{
	...
	char sex = 'M';
	for (node = DBList_GetFirstMatchNode(g_studentList, &sex, FindBySex); node != NULL;)
	{
		p_student = (Student_t*)DBList_GetNodeData(node);
		LOG_A("Find %s, '%c', %d.\n", p_student->p_name, p_student->sex, p_student->age);
		
		node = DBList_GetNextNode(node);
		node = DBList_GetNextMatchNode(g_studentList, node, &sex, FindBySex);
	}
	
	LOG_A("Find all 'M' from tail to head.\n");
	for (node = DBList_GetLastMatchNode(g_studentList, &sex, FindBySex); node != NULL;)
	{
		p_student = (Student_t*)DBList_GetNodeData(node);
		LOG_A("Find %s, '%c', %d.\n", p_student->p_name, p_student->sex, p_student->age);
		
		node = DBList_GetPreNode(node);
		node = DBList_GetPreMatchNode(g_studentList, node, &sex, FindBySex);
	}	
...
static void* InsertThread(void *p_param)
{
	...
	for (count  = 0; count < 10; count++)
	{
		...
		LOG_A("Insert:'%s'.\n", name);
		DBList_InsertData(g_studentList, p_student);
		sleep(1);
	}
	return NULL;
}
...
static void* FreeThread(void *p_param)
{
	...
	{
		p_student = DBList_GetHeadData(g_studentList);
		if (p_student != NULL)
		{
...
{
	DBList_t list = *((DBList_t*)(p_param));
	Student_t student;
	...
	for (i = 0; i < 10; i++)
	{
		...
		LOG_A("Insert %s.\n", student.p_name);
		DBList_Lock(list);
		DBList_InsertDataNL(list, &student);		
		DBList_UnLock(list);
...
static int TestMultiThreadLock()
{
	...
	DBList_t list;
	DBList_Create("TestList", sizeof(Student_t), &list);
